...
export default function SmallArabesque({ className }: Props) {
  // IDs uniques par instance (évite les collisions en prod)
  const uid = useId();
  const gradId = `${uid}-vaLine`;
  const glowId = `${uid}-vaGlow`;

  return (
    <svg
      viewBox="0 0 220 28"
      width="220"              // <= tailles explicites pour éviter un SVG 0x0
      height="28"
      className={className}

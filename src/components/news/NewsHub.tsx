...
export default function NewsHub() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await fetch(`/api/news-hub?cat=${encodeURIComponent(tab)}`, {
        cache: "no-store",
      });
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          {Array.from({ length: 9 }).map((_, i) => (
            <div
              key={i}
              className="h-44 rounded-2xl bg-white/70 ring-1 ring-[#f1ddcf] animate-pulse"
            />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <p className="opacity-80 mt-4">Aucun article trouvé.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
          {filtered.map((n) => (
            <a
            ...
            >
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-xs opacity-70">
                  {format(new Date(n.publishedAt), "d MMM yyyy", {
                    locale: fr,
                  })}
                </span>
              </div>
              ...
              {n.cats && n.cats.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {n.cats.slice(0, 3).map((c) => (
                    <span
                      key={c}
                      className="text-xs px-2 py-1 rounded-full ring-1 ring-[#e8cfc3] bg-white/70"
                    >
                      {c}
                    </span>
                  ))}

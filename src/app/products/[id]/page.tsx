...
// (optionnel) force le mode dynamique si besoin
...
export default async function ProductDetail({
  ...
  if (error || !product) {
    // 404 propre si pas trouvé
    notFound();
  }

  // 2) stats (facultatif; si tu as la view product_stats)
      ...
      <section className="mt-4 grid gap-6 md:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-2 text-sm text-gray-700 flex flex-wrap gap-3">
            ...
            {typeof product.rating === "number" && <span>Note fiche: {product.rating.toFixed(1)}/5</span>}
            {typeof stats?.avg_rating === "number" && <span>Moyenne avis: {Number(stats.avg_rating).toFixed(2)}⭐</span>}
            {typeof stats?.reviews_count === "number" && <span>{stats.reviews_count} avis</span>}
          </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border bg-green-50 p-3">
              ...
              <ul className="list-disc pl-4 text-sm text-green-900">
                {(product.highlights || []).slice(0, 6).map((x: string, i: number) => <li key={i}>{x}</li>)}
              </ul>
            </div>
            <div className="rounded-xl border bg-red-50 p-3">
              ...
              <ul className="list-disc pl-4 text-sm text-red-900">
                {(product.drawbacks || []).slice(0, 6).map((x: string, i: number) => <li key={i}>{x}</li>)}
              </ul>
            </div>
          </div>
          ...
          {(reviews || []).map((r: any) => (
            <div key={r.id} className="rounded-lg border p-3">
              <div className="text-sm text-gray-600 flex gap-2">
                ...
                {r.source && <span className="rounded bg-gray-100 px-2 py-0.5 text-xs">{r.source}</span>}
